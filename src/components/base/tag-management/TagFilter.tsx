...
const TagFilter: React.FC<TagFilterProps> = ({ type, value, onChange }) => {
  ...
  const filteredTagList = useMemo(() => {
    return data?.filter((tag) => tag.type === type && tag.name.includes(searchKeyword)) || [];
      ...
      <div className="relative w-[240px] rounded-lg border-[0.5] shadow-lg backdrop-blur-[5px] bg-[#fffffff2] border-[#10182814]">
        ...
        <div className="max-h-72 overflow-auto p-1">
          {filteredTagList.map((tag) => (
            ...
            >
              <div title={tag.name} className="grow truncate text-sm leading-5 color-[#676f83]">
                {tag.name}
              </div>
              {value.includes(tag.id) && <CheckOutlined className="h-4 w-4 shrink-0 text-[#354052]" />}
            </div>
          ))}
          {!filteredTagList.length && (
            <div className="flex flex-col items-center gap-1 p-3">
              <TagOutlined className="h-6 w-6 text-gray-400! text-xl" />
              <div className="text-xs leading-[14px] text-gray-400">{t('common.tag.noTag')}</div>
            </div>
          )}
        </div>
      </div>
    ...
    <Dropdown popupRender={renderDropDown} trigger={['click']}>
      <Button color="default" variant="filled">
        ...
        <Space>
          ...
          {value.length > 1 && (
            <div className="text-xs font-medium leading-[18px] text-[#676f83]">{`+${value.length - 1}`}</div>
          )}
          {!value.length && (
            <div className="p-px">

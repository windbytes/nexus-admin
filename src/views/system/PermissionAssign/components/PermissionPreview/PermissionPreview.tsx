...
import { DetailPermissionTabs, PermissionStatistics, RoleInfoCard, RoleSelectorBar } from './components';
...
const PermissionPreview: React.FC<PermissionPreviewProps> = memo(
  ({ roleCode: propRoleCode, showRoleSelector = true, showRefreshButton = true }) => {
     ...
     * 查询角色基本信息
     */
    const { data: roleInfo, isFetching: roleInfoLoading } = useQuery({
     ...
     * 查询角色权限详情
     */
    const { data: rolePermissions, isFetching: permissionsLoading } = useQuery({
     ...
     * 查询菜单权限详情
     */
    const { data: menuDetails, isFetching: menuDetailsLoading } = useQuery({
      queryKey: ['menu-details', rolePermissions?.menuPermissions],
      queryFn: () => {
        if (!rolePermissions?.menuPermissions?.length) {
          return [];
        }
        // 这里应该根据菜单ID查询菜单详情
     ...
     * 查询按钮权限详情
     */
    const { data: buttonDetails, isFetching: buttonDetailsLoading } = useQuery({
      queryKey: ['button-details', rolePermissions?.buttonPermissions],
      queryFn: () => {
        if (!rolePermissions?.buttonPermissions?.length) {
          return [];
        }
        return permissionButtonService
          .getButtonList({})
          .then((response) => response.filter((button: any) => rolePermissions.buttonPermissions?.includes(button.id)));
      },

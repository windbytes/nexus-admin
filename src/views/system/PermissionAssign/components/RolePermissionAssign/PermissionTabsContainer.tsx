...
}>(({ checkedKeys, onCheck }) => <MenuPermissionTree checkedKeys={checkedKeys} onCheck={onCheck} />);
...
const ButtonPermissionTreeWrapper = memo<{
  checkedKeys: string[];
  onCheck: (keys: string[]) => void;
}>(({ checkedKeys, onCheck }) => <ButtonPermissionTree checkedKeys={checkedKeys} onCheck={onCheck} />);
...
const InterfacePermissionGridWrapper = memo<{
  checkedKeys: string[];
  onCheck: (keys: string[]) => void;
}>(({ checkedKeys, onCheck }) => <InterfacePermissionGrid checkedKeys={checkedKeys} onCheck={onCheck} />);
...
const PermissionTabsContainer: React.FC<PermissionTabsContainerProps> = memo(
  ({
     ...
     */
    const renderPermissionContent = useCallback(
      (permissionComponent: React.ReactNode) => {
        if (!currentRoleCode) {
          return (
        ...
        return <div className="h-full overflow-hidden">{permissionComponent}</div>;
      },

...
function dropdownPlaceholder(label: string) {
  return () =>
    React.createElement('div', { className: 'py-2 px-3 text-sm text-gray-500' }, `${label} - 下拉占位`);
}
...
const startGroups: ToolGroupConfig[] = [
  {
    ...
    rows: 1,
    tools: [
      { key: 'formatPainter', label: '格式刷', type: 'button', command: 'format.painter' },
      { key: 'paste', label: '粘贴', type: 'dropdown', command: 'edit.paste', dropdownRender: dropdownFromPlugin('paste', '粘贴') },
      { key: 'cut', label: '剪切', type: 'button', command: 'edit.cut' },
    ],
  },
  {
    key: 'font',
    ...
    tools: [
      ...
      { key: 'case', label: '大小写', type: 'dropdown', command: 'format.case', dropdownRender: dropdownPlaceholder('大小写'), rowIndex: 0 },
      ...
      { key: 'underline', label: 'U', type: 'dropdown', command: 'format.underline', dropdownRender: dropdownPlaceholder('下划线'), rowIndex: 1 },
      { key: 'strikethrough', label: '删除线', type: 'dropdown', command: 'format.strikethrough', dropdownRender: dropdownPlaceholder('删除线'), rowIndex: 1 },
      { key: 'highlight', label: '突出显示', type: 'dropdown', command: 'format.highlight', dropdownRender: dropdownPlaceholder('突出显示'), rowIndex: 1 },
      { key: 'fontColor', label: '字体颜色', type: 'dropdown', command: 'format.fontColor', dropdownRender: fontColorDropdownRender, rowIndex: 1 },
    ],
  },
  {
    key: 'paragraph',
    rows: 2,
    tools: [
      { key: 'bulletList', label: '项目符号', type: 'dropdown', command: 'format.bulletList', dropdownRender: dropdownPlaceholder('项目符号'), rowIndex: 0 },
      { key: 'numberList', label: '编号', type: 'dropdown', command: 'format.numberList', dropdownRender: dropdownPlaceholder('编号'), rowIndex: 0 },
  ...
  },
  {
    ...
    tools: [
      ...
      { key: 'styleSet', label: '样式集', type: 'dropdown', command: 'style.set', dropdownRender: dropdownPlaceholder('样式集') },
    ],
  },
  {
    key: 'editing',
    rows: 1,
    tools: [
      { key: 'findReplace', label: '查找替换', type: 'dropdown', command: 'edit.findReplace', dropdownRender: dropdownPlaceholder('查找替换') },
      { key: 'select', label: '选择', type: 'dropdown', command: 'edit.select', dropdownRender: dropdownPlaceholder('选择') },
    ],
  },
];
...
const insertGroups: ToolGroupConfig[] = [
  {
    ...
    rows: 2,
    tools: [
      { key: 'blankPage', label: '空白页', type: 'button', command: 'insert.blankPage', rowIndex: 0 },
      { key: 'cover', label: '封面', type: 'dropdown', command: 'insert.cover', dropdownRender: dropdownPlaceholder('封面'), rowIndex: 0 },
      { key: 'pageBreak', label: '分页', type: 'button', command: 'insert.pageBreak', rowIndex: 1 },
      { key: 'pageNumber', label: '页码', type: 'dropdown', command: 'insert.pageNumber', dropdownRender: dropdownPlaceholder('页码'), rowIndex: 1 },
  ...
  },
  {
    ...
    rows: 2,
    tools: [
      { key: 'table', label: '表格', type: 'dropdown', command: 'insert.table', dropdownRender: dropdownPlaceholder('表格'), rowIndex: 0 },
      { key: 'image', label: '图片', type: 'dropdown', command: 'insert.image', dropdownRender: dropdownPlaceholder('图片'), rowIndex: 0 },
      { key: 'shape', label: '形状', type: 'dropdown', command: 'insert.shape', dropdownRender: dropdownPlaceholder('形状'), rowIndex: 1 },
      { key: 'textBox', label: '文本框', type: 'dropdown', command: 'insert.textBox', dropdownRender: dropdownPlaceholder('文本框'), rowIndex: 1 },
  ...
  },
  {
    ...
    rows: 1,
    tools: [
      { key: 'symbol', label: '符号', type: 'dropdown', command: 'insert.symbol', dropdownRender: dropdownPlaceholder('符号') },
      { key: 'equation', label: '公式', type: 'dropdown', command: 'insert.equation', dropdownRender: dropdownPlaceholder('公式') },
    ],
  },
  {
    key: 'link',
...
const pageGroups: ToolGroupConfig[] = [
  ...
  },
  {
    ...
    rows: 1,
    tools: [
      { key: 'orientation', label: '纸张方向', type: 'dropdown', command: 'page.orientation', dropdownRender: dropdownPlaceholder('纸张方向') },
      { key: 'paperSize', label: '纸张大小', type: 'dropdown', command: 'page.paperSize', dropdownRender: dropdownPlaceholder('纸张大小') },
    ],
  },
  {
    key: 'appearance',
    rows: 1,
    tools: [
      { key: 'theme', label: '主题', type: 'dropdown', command: 'page.theme', dropdownRender: dropdownPlaceholder('主题') },
      { key: 'watermark', label: '水印', type: 'dropdown', command: 'page.watermark', dropdownRender: dropdownPlaceholder('水印') },
    ],
  },
];
...
const reviewGroups: ToolGroupConfig[] = [
  {
    ...
    tools: [
      ...
      { key: 'track', label: '修订', type: 'dropdown', command: 'review.track', dropdownRender: dropdownPlaceholder('修订') },
      { key: 'accept', label: '接受', type: 'dropdown', command: 'review.accept', dropdownRender: dropdownPlaceholder('接受') },
      { key: 'reject', label: '拒绝', type: 'dropdown', command: 'review.reject', dropdownRender: dropdownPlaceholder('拒绝') },
    ],
  },
];
